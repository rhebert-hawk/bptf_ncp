...
y,yerr = np.loadtxt("NucE_h3_1.dat", usecols=(1,2), unpack=True, comments=('#','@','&'))
f,y1,y1err = np.loadtxt("NucE_h3_2.dat", usecols=(0,1,2), unpack=True, comments=('#','@','&'))
x = np.array([i for i in range(0,135)])


fig,ax=plt.subplots(figsize=(8.0,4.0))
plt.plot(x, y, color = 'orange',label = "H3K4me3 (Tail 1)")
plt.plot(x, y1, color = 'black', label = "H (Tail 2)")
...
plt.tight_layout()
plt.title("Nucleosome + BPTF (II)", fontweight='bold')

plt.savefig("Eh3.svg",format='svg')
plt.show()
plt.close()
