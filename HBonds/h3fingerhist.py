...
y = np.loadtxt("EnoBall_h3bptf.dat", usecols=(1), unpack=True, comments=('#','@','&'))
z = np.loadtxt("Eall_h3bptf.dat", usecols=(1), unpack=True, comments=('#','@','&'))
...
plt.hist(y, alpha=0.5, bins=74, density=True, label='PHD + NCP', histtype='step')
plt.hist(z, alpha=0.5, bins=89, density=True, label='BPTF + NCP', histtype='step')
plt.xlabel('Number of Hydrogen Bonds', fontdict=font)
plt.ylabel('Frequency', fontdict=font)
plt.xticks(fontsize=15)
plt.yticks(fontsize=15)
plt.title('H3 and PHD Finger Hydrogen Bonds', fontdict=font)
