...
x = np.loadtxt("Nucall_nuc.dat", usecols=(1), unpack=True, comments=('#','@','&'))
	#The only difference this time is since we are plotting histograms, we only need one data column
y = np.loadtxt("EnoBall_all.dat", usecols=(1), unpack=True, comments=('#','@','&'))
z = np.loadtxt("Eall_all.dat", usecols=(1), unpack=True, comments=('#','@','&'))
plt.hist(x, alpha=0.5, bins=152, density=True, label='NCP', histtype='step')
...
plt.hist(y, alpha=0.5, bins=190, density=True, label='PHD + NCP', histtype='step')
plt.hist(z, alpha=0.5, bins=220, density=True, label='BPTF + NCP', histtype='step')
plt.xlabel('Number of Hydrogen Bonds', fontdict=font)
plt.ylabel('Frequency', fontdict=font)
plt.xticks(fontsize=15)
plt.yticks(fontsize=15)
plt.title('Total Hydrogen Bonds', fontdict=font)
